...
# package, for the Amiga-side TLS transport (src/amiga/transport_amissl.c,
# issue #3 Phase 3). Feature-detected, not required by the default build -
# see the Makefile's M68K_HAS_AMISSL comment: `make library`/`make m68k`
# build without TLS support unless this has been run first (matching
# HOST_HAS_TLS's same idiom on the host side).
#
# Fetches two archives from the same pinned release:
#   - AmiSSL-<ver>-SDK.lha  — headers + libamisslstubs.a, for linking
#     mqtt.library against m68k-amigaos-gcc.
#   - AmiSSL-<ver>-OS3.lha  — the actual runtime amisslmaster.library and the
#     CPU-specific amissl_v*.library, needed on-target (staged onto a boot
#     volume's LIBS:/LIBS:AmiSSL/) for OpenLibrary() to succeed - not needed
#     to build, only to run something linked against AmiSSL.
#
...
#   AMISSL_CACHE_DIR  cache root (default .cache/amissl-sdk, relative to the
#                      repo root - see .gitignore's /.cache/ entry - so a
#                      Docker-mounted cross-build sees the same cache the
#                      host does without needing $HOME inside the container)
#
...
AMISSL_CACHE_DIR="${AMISSL_CACHE_DIR:-$HERE/.cache/amissl-sdk}"
